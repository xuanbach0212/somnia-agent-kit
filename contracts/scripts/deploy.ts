...
async function main() {
  ...
  // Set agent registry address
  console.log('🔧 Setting AgentRegistry address in AgentManager...');
  await agentManager.setAgentRegistry(agentRegistryAddress);
  console.log('✅ AgentRegistry address set');
  ...
  console.log(
    `   pnpm --filter contracts run verify ${agentRegistryAddress} --network ${network.name}`
  );
